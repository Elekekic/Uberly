...
export class ReplyService {
  ...
  createReply(reply: Reply): Observable<Reply> {
    return this.http.post<Reply>(`${this.apiURL}/replies`, reply, { responseType: 'json' }).pipe(
      tap(() => {
        if (reply.commentId) {
  ...
  updateReply(id: number, reply: Reply): Observable<Reply> {
    return this.http.put<Reply>(`${this.apiURL}/replies/${id}`, reply, { responseType: 'json' }).pipe(
      tap(() => {
        if (reply.commentId) {
          this.refreshRepliesByComment(reply.commentId);
        }
  ...
  deleteReply(id: number): Observable<void> {
    return this.http.delete<void>(`${this.apiURL}/replies/${id}`, { responseType: 'text' as 'json' }).pipe(
      tap(() => {
