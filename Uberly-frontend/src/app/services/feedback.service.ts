...
export class FeedbackService {
  ...
  createFeedback(feedbackDTO: FeedbackDTO): Observable<Feedbacks> {
    return this.http.post<Feedbacks>(`${this.apiURL}/feedbacks`, feedbackDTO, { responseType: 'text' as 'json' }).pipe(
      tap(() => {
  ...
  updateFeedback(feedBackId: number, feedbackUpdateDTO: Feedbacks): Observable<string> {
    return this.http.put<string>(`${this.apiURL}/feedbacks/${feedBackId}`, feedbackUpdateDTO, { responseType: 'text' as 'json' }).pipe(
      tap(() => {
        const authorFeedbackIndex = this.feedbacksAuthor.findIndex(feedback => feedback.id === feedBackId);
        const recipientFeedbackIndex = this.feedbacksRecipient.findIndex(feedback => feedback.id === feedBackId);
  ...
  getFeedbacksByAuthorId(authorId: number): Observable<Feedbacks[]> {
    return this.http.get<Feedbacks[]>(`${this.apiURL}/feedbacks/author/${authorId}`).pipe(
      tap(feedbacks => {
  ...
  getFeedbacksByRecipientId(recipientId: number): Observable<Feedbacks[]> {
    return this.http.get<Feedbacks[]>(`${this.apiURL}/feedbacks/recipient/${recipientId}`).pipe(
      tap(feedbacks => {
  ...
  deleteFeedback(feedBackId: number): Observable<string> {
    return this.http.delete<string>(`${this.apiURL}/feedbacks/${feedBackId}`, { responseType: 'text' as 'json' }).pipe(
      tap(() => {
        const authorFeedbackIndex = this.feedbacksAuthor.findIndex(feedback => feedback.id === feedBackId);
        const recipientFeedbackIndex = this.feedbacksRecipient.findIndex(feedback => feedback.id === feedBackId);
        if (recipientFeedbackIndex !== -1 && authorFeedbackIndex !== -1 ) {
